...
(***************************************************************************)
    ...
    and WorkItem = { Eff: FIO<obj, obj>; LLFiber: LowLevelFiber } with
    ...
    and EvalWorker(id, runtime : Advanced, 
            ...
            blockingQueue : BlockingCollection<BlockingItem>, execSteps) =
        let _ = (async {
     ...
     and BlockingWorker(id, workQueue : BlockingCollection<WorkItem>, blockingQueue : BlockingCollection<BlockingItem>) =
        let _ = (async {
    ...
    and Advanced(?evalWorkerCount, ?blockingWorkerCount, ?evalSteps) as self =
        ...
        member private _.CreateWorkers () =
            ...
            | Some evalWorkerCount -> 
                ...
                | Some blockingWorkerCount -> createBlockingWorkers evalWorkerCount (evalWorkerCount + blockingWorkerCount - 1) |> ignore
                | _                        -> createBlockingWorkers evalWorkerCount (evalWorkerCount + defaultBlockingWorkerCount - 1) |> ignore
            | _                    ->
                createEvalWorkers 0 (defaultEvalWorkerCount - 1) |> ignore
                match blockingWorkerCount with
