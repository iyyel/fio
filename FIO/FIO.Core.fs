...
and internal WorkItem =
    ...
    member internal this.Complete result = this.IFiber.Complete result
...
and internal InternalFiber internal (resultQueue: InternalQueue<Result<obj, obj>>, blockingWorkItemQueue: InternalQueue<WorkItem>) =
    ...
    member internal this.Completed() = resultQueue.Count > 0

    member internal this.AddBlockingWorkItem workItem = blockingWorkItemQueue.Add workItem

    member internal this.BlockingWorkItemsCount() = blockingWorkItemQueue.Count
...
and Channel<'D> private (dataQueue: InternalQueue<obj>, blockingWorkItemQueue: InternalQueue<WorkItem>) =
    ...
    member internal this.HasBlockingWorkItems() = blockingWorkItemQueue.Count > 0

    member internal this.Upcast() =
        Channel<obj>(dataQueue, blockingWorkItemQueue)

    member this.Add(data: 'D) = dataQueue.Add data

    member this.Take() : 'D = dataQueue.Take() :?> 'D

    member this.Count() = dataQueue.Count
...
and FIO<'R, 'E> =
    ...
    member internal this.UpcastResult<'R, 'E>() : FIO<obj, 'E> =
        ...
        | Blocking channel -> Blocking <| channel.Upcast()
        | Send(message, channel) -> Send(message :> obj, channel.Upcast())
        | Concurrent(effect, fiber, ifiber) -> Concurrent(effect, fiber, ifiber)
        | Await ifiber -> Await ifiber
        | SequenceSuccess(effect, continuation) -> SequenceSuccess(effect, fun result -> (continuation result).UpcastResult())
        | SequenceError(effect, continuation) -> SequenceError(effect, fun result -> (continuation result).UpcastResult())
        | Success result -> Success(result :> obj)
    ...
    member internal this.UpcastError<'R, 'E>() : FIO<'R, obj> =
        ...
        | Send(message, channel) -> Send(message, channel)
        | Concurrent(effect, fiber, ifiber) -> Concurrent(effect, fiber, ifiber)
        | Await ifiber -> Await ifiber
        | SequenceSuccess(effect, continuation) -> SequenceSuccess(effect.UpcastError(), fun result -> (continuation result).UpcastError())
        | SequenceError(effect, continuation) -> SequenceError(effect.UpcastError(), fun result -> (continuation result).UpcastError())
...
let send<'R, 'E> (message: 'R) (channel: Channel<'R>) : FIO<'R, 'E> = Send(message, channel)

/// *> is an alias of send which puts a message on the channel argument when interpreted.
/// The effect succeeds with the message argument ('R).
let ( *> ) (message: 'R) (channel: Channel<'R>) : FIO<'R, 'E> = send message channel
...
let receive<'R, 'E> (channel: Channel<'R>) : FIO<'R, 'E> = Blocking channel

/// !*> is an alias of receive which awaits message retrieval on the channel argument when interpreted.
/// The effect succeeds with the received message argument ('R).
let (!*>) (channel: Channel<'R>) : FIO<'R, 'E> = receive channel
...
/// The effect succeeds with the associated fiber which can be awaited to retrieve the result.
let (!>) (effect: FIO<'R1, 'E1>) : FIO<Fiber<'R1, 'E1>, 'E> = concurrently effect
...
let await<'R, 'E> (fiber: Fiber<'R, 'E>) : FIO<'R, 'E> = Await <| fiber.ToInternal()

/// !?> is an alias of await which awaits the result of the fiber argument when interpreted.
/// The effect succeeds with the result of the fiber argument ('R).
let (!?>) (fiber: Fiber<'R, 'E>) : FIO<'R, 'E> = await fiber
...
/// to the continuation function when interpreted.
/// If an error ('E) occurs the effect returns immediately.
let (>>) (effect: FIO<'R1, 'E>) (continuation: 'R1 -> FIO<'R, 'E>) : FIO<'R, 'E> = sequenceSuccess effect continuation
...
/// to the continuation function when interpreted.
/// If a success ('R) occurs the effect returns immediately. TODO: Is this part true?
let (?>) (effect: FIO<'R, 'E1>) (continuation: 'E1 -> FIO<'R, 'E>) : FIO<'R, 'E> = sequenceError effect continuation
...
let parallelize<'R1, 'R2, 'E> (firstEffect: FIO<'R1, 'E>) (secondEffect: FIO<'R2, 'E>) : FIO<'R1 * 'R2, 'E> =
    ...
    >> fun secondFiber ->
        firstEffect
        >> fun firstResult -> !?>secondFiber >> fun secondResult -> !+(firstResult, secondResult)

/// <*> is an alias of parallelize which executes the two effect arguments in parallel when interpreted.
/// The effects succeeds with the results of the two effect arguments in a tuple ('R1 * 'R2).
/// If an error ('E) occurs the effect returns immediately.
let (<*>) (leftEffect: FIO<'R1, 'E>) (rightEffect: FIO<'R2, 'E>) : FIO<'R1 * 'R2, 'E> = parallelize leftEffect rightEffect
...
let zip<'R1, 'R2, 'E> (firstEffect: FIO<'R1, 'E>) (secondEffect: FIO<'R2, 'E>) : FIO<'R1 * 'R2, 'E> =
    firstEffect
    >> fun firstResult -> secondEffect >> fun secondResult -> !+(firstResult, secondResult)

/// <^> is an alias of zip which succeeds with the results of the effect arguments ('R1, 'R2) in a tuple ('R1 * 'R2) when interpreted.
/// If an error ('E) occurs the effect returns immediately.
let (<^>) (leftEffect: FIO<'R1, 'E>) (rightEffect: FIO<'R2, 'E>) : FIO<'R1 * 'R2, 'E> = zip leftEffect rightEffect
...
/// where the result of the effect that completes first ('R) is returned on success when interpreted.
/// If an error ('E) occurs the effect returns immediately.
let (<?>) (leftEffect: FIO<'R, 'E>) (rightEffect: FIO<'R, 'E>) : FIO<'R, 'E> = race leftEffect rightEffect
...
module internal FIOBuilderHelper =
    ...
    let inline Bind (effect: FIO<'R1, 'E>) (continuation: 'R1 -> FIO<'R, 'E>) : FIO<'R, 'E> = effect >> continuation

    /// Wraps a value in a successful FIO computation.
    let inline Return (result: 'R) : FIO<'R, 'E> = !+result

    /// Directly returns an existing FIO computation.
    let inline ReturnFrom (effect: FIO<'R, 'E>) : FIO<'R, 'E> = effect

    /// Combines two computations, running one after the other.
    let inline Combine (firstEffect: FIO<'R, 'E>) (secondEffect: FIO<'R1, 'E>) : FIO<'R1, 'E> = firstEffect >> fun _ -> secondEffect
    ...
    let inline Run (effect: FIO<'R, 'E>) : FIO<'R, 'E> = effect

    /// Handles failure cases in the effect using the provided handler.
    let inline TryWith (effect: FIO<'R, 'E>) (handler: 'E -> FIO<'R, 'E>) : FIO<'R, 'E> = effect ?> handler

    /// Ensures a finalizer is executed after the main computation, regardless of success or failure.
    let inline TryFinally (effect: FIO<'R, 'E>) (finalizer: unit -> unit) : FIO<'R, 'E> =
        effect
        >> fun result ->
            try
                finalizer ()
    ...
    let While (guard: unit -> bool, effect: FIO<'R, 'E>) : FIO<Unit, 'E> =
        let rec loop () =
            if guard () then effect >> fun _ -> loop () else succeed ()
...
type FIOBuilder() =
    ...
    member this.While(guard: unit -> bool, effect: FIO<'R, 'E>) : FIO<Unit, 'E> = FIOBuilderHelper.While(guard, effect)
