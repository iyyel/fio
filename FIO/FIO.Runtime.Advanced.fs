...
and AdvancedRuntime(evalWorkerCount, blockingWorkerCount, evalStepCount) as self =
...
#endif
#if MONITOR
        Monitor(workItemQueue, None, Some blockingEventQueue, None) |> ignore
#endif
    ...
    member internal this.InternalRun effect prevAction evalSteps stack : (FIO<obj, obj> * Stack) * Action * int =
        ...
        if evalSteps = 0 then
            ...
            | SequenceSuccess(effect, continuation) -> this.InternalRun effect prevAction evalSteps (SuccConts continuation :: stack)
            | SequenceError(effect, continuation) -> this.InternalRun effect prevAction evalSteps (ErrorConts continuation :: stack)
    ...
    member private this.CreateBlockingWorkers() : BlockingWorker list =
        let createBlockingWorkers start final =
            List.map
                (fun _ ->
#if DETECT_DEADLOCK
                    BlockingWorker(workItemQueue, deadlockDetector, blockingEventQueue))
                [ start..final ]
#else
