...
type RuntimeTests() =
    ...
    let getSuccessResult(result : Result<'R, 'E>, expected : 'R) =
        match result with
        | Ok result -> result
        | Error error -> Assert.Fail($"Result contained Error ({error}) when it was expected to contain Ok ({expected})")
                         failwith "Test failed"

    let getFailureResult(result : Result<'R, 'E>, expected : 'E) =
        match result with
        | Ok result -> Assert.Fail($"Result contained Ok ({result}) when it was expected to contain Error ({expected})")
                       failwith "Test failed"
        | Error error -> error
    ...
    member this.SequenceErrorSuccessFunctionTest(runtime : Runtime) =
        ...
        let effect = fail 42 ?> fun _ -> 
                     succeed "will not succeed" >> fun _ -> 
                     fail expected
